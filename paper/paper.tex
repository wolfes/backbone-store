...
douments, medium-sized emails, and small notes from both the localStorage on
the client's browser and on the server. We find that localStorage performs
faster than loading the same data from th server. This time difference
...
real-world usecases where loading data from a server would cause users to
percieve lag, while loading the same data from localStorage would be percieved
as instantaneous.
\end{abstract}
...
This provides flexibility for different usecases where one option may be
...
Second, we conduct a performance evaluation of load and update times for data
...
We find that our API provides superior load and update performance for data
stored locally vs.\ remotely under all data size and quantity conditions except
...
To evaluate the benefits of Backbone-store, we build three test applications to
...
paragraphs of lorem-ipsum, totalling 3265 characters per item. To examine
medium size items such as emails, we used one paragraph of lorem-ipsum,
totallying 905 characters per item. To examine smaller items such as personal
notes, we used a short string of 70 characters. These character lengths are the
length of the JSON string that represents each item. The three applications
performed the same task: load some number of a particular type of item from
localStorage and from the server, separated by a delay so as not to overlap in
execution and skew the load time, and record the load time.
...
is complete. To record the time to load the same data from localStorage, the
timer is started when just before the \verb=loadFromStore= method is called by
a collection. The data is then fetched from localStorage and parsed from a JSON
...
Finally, we also tested update times for our two persistence strategies. We did
this by creating collections with different numbers of items, and measuring the
time to save each collection to localStorage. Also, to test different save-time
...
one-key-per-model persistence strategy will allow faster updates, by making it
...
300 items per follection) and show that local caching performs faster than data
...
persistence strategies, and from the server, grow linearly with the number of
