...
we extended Backbone.js to create our own local caching API to help web developers easily store and reload a web application's data using the HTML5 localStorage API. To evaluate the API,  we analyze three simple applications to measure the performance characteristics of loading large
...
increases for larger quantities of the same type of item, suggesting that local storage become more advantageous as the amount of loaded data increases. Furthermore, given that the threshold
...
with HTML5. Web SQL \cite{websql} presents a database API similar to SQLite and provides
...
key-value storage options, called sessionStorage and localStorage \cite{localstorage}, that have a
